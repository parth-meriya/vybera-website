...
const CategoryPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setSectionLabel(currentSection ? currentSection.label : slug.replace('-', ' '));

        // Fetch products
        const p = await getProductsByCategory(slug);
        setProducts(p);
        setFiltered(p);
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-vy-black pt-24">
      <SEO
        title={`${sectionLabel} Collection`}
        description={`Explore the exclusive ${sectionLabel} collection at VYBERA.`}
        keywords={`${sectionLabel}, VYBERA`}
      ...
      <div className="max-w-screen-xl mx-auto px-6 md:px-12">
        ...
        >
          <div>
            <p className="text-vy-grey text-xs tracking-[0.5em] uppercase mb-1">Collection</p>
            <h1 className="font-display font-bold text-3xl md:text-4xl tracking-wider text-vy-white uppercase">
              {sectionLabel}
            </h1>
          </div>
          <div className="flex items-center gap-4">
            ...
            <span className="text-vy-grey text-xs hidden md:inline">{filtered.length} items</span>
          </div>
        </motion.div>
        ...
        {loading ? (
          ...
          <div className="text-center py-32">
            <p className="text-vy-grey text-sm tracking-widest uppercase mb-4">No products found in this collection.</p>
            {hasFilters ? (
              <button onClick={clearFilters} className="btn-outline text-xs">Clear Filters</button>
            ) : (
              <p className="text-vy-border text-xs">Stay tuned! Products are coming soon.</p>
            )}
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6 pb-20">
