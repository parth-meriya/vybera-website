...
const Orders = () => {
  ...
  const getStatusColor = (status) => {
    switch(status?.toLowerCase()) {
      ...
      case 'out_for_delivery': return 'text-yellow-400 border-yellow-500/30 bg-yellow-500/10';
      case 'delivered': return 'text-green-400 border-green-500/30 bg-green-500/10';
      case 'processing': return 'text-orange-400 border-orange-500/30 bg-orange-500/10';
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
      <div>
        <h2 className="font-display font-bold text-3xl tracking-wider mb-2">My Orders</h2>
              ...
              <div className="p-6">
                <div className="flex flex-col md:flex-row gap-6 justify-between items-start">
                  <div className="space-y-4 flex-1">
                    ...
                    <div className="flex flex-wrap items-center gap-6 pt-4 border-t border-vy-border/30">
                        <div>
                        ...
                        {order.pointsEarned > 0 && (
                          <div>
                            <p className="text-vy-grey text-[9px] uppercase tracking-widest mb-1">Points Earned</p>
                            <p className="text-green-400 text-xs font-bold">+{order.pointsEarned}</p>
                          </div>
                        )}
                        {order.pointsRedeemed > 0 && (
                          <div>
                            <p className="text-vy-grey text-[9px] uppercase tracking-widest mb-1">Points Used</p>
                            <p className="text-vy-accent text-xs font-bold">-{order.pointsRedeemed}</p>
                          </div>
                        )}
                    </div>
                  </div>
